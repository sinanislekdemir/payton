...
class RagDoll(Bone):
    ...
    def setup_joints(self):
        neck = Bone(
            root_joint_name="lower_neck", end_joint_name="head", length=0.2
        )
        self.connect_to_end(neck.root)
        head = Bone(
            root_joint_name="head", end_joint_name="head_top", length=0.4
        )
        ...
        left_hand = Bone(
            root_joint_name="l_wrist", end_joint_name="l_hand", length=0.2
        )
        ...
        right_foot = Bone(
            root_joint_name="r_foot", end_joint_name="r_toe", length=0.3
        )
        right_lower_leg.connect_to_end(right_foot.root)
        right_foot.root.rotate_around_x(math.radians(90))

        left_foot = Bone(
            root_joint_name="l_foot", end_joint_name="l_toe", length=0.3
        )
